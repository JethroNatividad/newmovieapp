    ...
    return (
        <nav className="relative">
            ...
            <div className="flex px-10 sm:px-20 space-x-10 sm:space-x-20 overflow-y-hidden overflow-x-scroll h-auto hide-scroll text-2xl sm:text-3xl">
                ...
                {navItems.map(({ id, title, endpoint }) => (<p key={id} onClick={() => handleNavClick(id)} className={`hover:text-orange-200 hover:scale-125 py-2 cursor-pointer transition-all whitespace-nowrap ${currentGenre === id && 'text-orange-300 scale-125'}`}>{title}</p>))}
